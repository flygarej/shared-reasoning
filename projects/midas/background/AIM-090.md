<!--
AIM-090 Markdown facsimile, first pass.
Source text: AIM-090-transcribed(1).txt
Visual authority: AIM-090.pdf
The transcription preserves original spelling/punctuation; this file intentionally does not modernize them.
-->
...
> This is a transcription of MIT Project MAC Artificial Intelligence Memo No. 90, "MIDAS", by Peter Samson.

> The scanned document is authoritative. Original spelling, punctuation, capitalization, and apparent typographical errors are preserved.

> Text supplied or reconstructed by the transcriber is explicitly marked. A question mark <?> following reconstructed text indicates uncertainty.

> Page breaks in the source are preserved.

> Several handdrawn typgraphical symbols are in the PDF, those have been given a tag and are referred in text as <tag>, exception is when space is explicitly marked in text, then _ (underscore) is used.


**Glyph key used in this Markdown edition:** `↵` = original bent return/down-arrow; `△` = triangle separator; `▷` = right-facing triangle; `↓` = down arrow; `←` = left arrow. Hoverable HTML spans retain the transcription tag meaning.
...
<pre>
		MASSACHUSETTS INSTITUTE OF TECHNOLOGY
			      Project MAC
						...
						January 31, 1966
TO:    PDP-6 USERS				Attached to A.I. Memo. 90
FROM:  Peter Samson
...
the command characters said to have a value. $⃝ is ALT mode, which
echoes out as $. _ represents SPACE.

<ins>command form</ins>  	   <ins>Meaning</ins>
...
nMname1,name2$⃝ $⃝   Set to read from the beginning of file <ins>name1, name2</ins> on
...
<pre>
(Transcriber note, if there are commands for the functions on this page,
the command is missing in the original PDF, suggested command letters
listed with a &lt;?&gt; if not known &lt;missing&gt; is used)

					-2-

L&lt;?&gt;   	       	   Load selected input file, saving local symbol definitions
		   	for DDT. (N and L set the Current Starting
			Address to that specified in the program
			loaded if that is not 0.

C&lt;?&gt;		   Copy all defined global symbols in the Loader Table
		   	into the local symbol table (for DDT); then
			delete same from the Loader Table.

R&lt;?&gt;		   Read in the relocatable version of DDT from DECtape
		   	unit 1, tell it of all symbols in the local
			symbol table; wipe out the loader and transfer
			control to DDT.

nL&lt;?&gt;		   List files of DECtape unit n. (As in the M command,
		   	the argument n may be omitted.)

T&lt;?&gt;		   Transfer control to the Current STarting Address.

S&lt;?&gt;		   Set the Current Starting Address to n and transfer
		       control thereto.

&lt;missing&gt;	   Has the value of the Current Starting Address

&lt;missing&gt;	   Has the value of the lowest address currently used
		       by the loader

D&lt;?&gt;		   Delete all local and global symbols from the local
		       symbol table and the Loader Table.

Z&lt;?&gt;		   Zero core except registers 20 through 37 and the loader
		   	from E up.

nP&lt;?&gt;		   Print contents of location n.

&lt;missing&gt;	   Has the current value of program relocation.

&lt;missing&gt;	   Set program relocation to n.

&lt;missing&gt;	   Set common relocation to n.

&lt;missing&gt;	   Print storage map: Each program in core has one line
...
n?		   Print storage map and missing list (long form). (Here
		   	 the value of n is immaterial, but one argument
...
n&lt;sym&gt;		   Define symbol sym with value n. The symbol will be
		   	 global unless a * is typed somewhere between
			 &lt; and &gt;.

Error message	 meaning
SCE adr		 Storage capacity exceeded. The program being loaded
...
.LIBRA pseudoinstruction, and so will be loaded only if in the Loader
...
loaded that make reference to the library subroutines.

Command string examples:

(a1)	To load the program APLHA RALPHA from DECtape unit 3, the program
	SUBR 1 from unit 2 and the program BR 2 from unit 2; then to
	(Transcriber note: Original APLHA probably ALPHA and BR should be SUBR 2)
	get a storage map and missing list:

	3MALPHA RALPHA $⃝ L2MSUBR 1 $⃝ LMSUBR 2 $⃝ L? $⃝ $⃝
...
(c)	To load PROG REL from DECtape unit 1 and the requested library
	...
	1MPROG REL $⃝ L4MLIBRAR 1 $⃝ N $⃝ $⃝
</pre>
...
according to the MAC file format (see MAC-M-249). The output of MIDAS is
...
in the input does not depend on its position in a line or the page, but
instead upon the characters (such as colon, comma, and carriage return) which
delimit it.
The output of MIDAS consists primarily of <ins>storage words</ins>: 36-bit quantities
intended to be placed in memory locations when the file is loaded. Also in
the output file usually a <ins>symbol table</ins> -- a list of names and values of all
...
of them, <ins>relocatable</ins>, allows him to make symbolic cross-references between
...
(Transcriber note leftarrow/uparrow are hand drawn arrows in the manual...)
←    	|
...
The elementary unit of meaning in MIDAS is the syllable. Depending on its
form and upon how it was defined, a syllable may be a <ins>number</ins>, a <ins>symbol</ins>,
a <ins>macro name</ins>, a <ins>pseudoinstruction</ins>, a <ins>quoted character</ins> or one of several kinds
of <ins>bracketed word</ins>. A pseudoinstruction directs some action on the part of
MIDAS; a macro name stands for a string of characters; other syllables (termed
<ins>valued</ins> <ins>syllables</ins>) represent numeric quantities, which are 36 bits in size.
Such a quantity is the <ins>value</ins> of the syllable. Associated with each valued
syllable is a relocation: a quantity which is either 0 or 1. The value of
some syllables may be <ins>virtual</ins>, meaning the value is not known at assembly
time but will become known as the program is loaded. Virtual quantities have
a relocation of 0.

Numbers

A string of digits forms an <ins>integer</ins> with its expressed numeric value and 0
relocation. The number is interpreted in the <ins>current radix</ins>. An integer
...
by an integer:
   ...
   A*R<sub>A</sub>ᴮ with R<sub>A</sub> being the radix in which A is expressed.
A string of digits with . (period) to the left of some digit is a <ins>floating-point
number</ins>; decimal radix is assumed. A floating point number may be followed by
↑ integer, and the result is a floating point number: eg 3.5 ↑ 4 means the
</pre>
...
same as 35000.0. Any of these numeric formats may be followed by (leftarrow) integer,
which multiplies the current value by 2<sup>INTEGER</sup>. (The exponent integer may
be terminated by &#x27; or . to force its radix; otherwise it used the current radix.)
The result is a fixed-point integer! For instance, 1.5 (leftarrow) 3 = 14&#x27;; and
17&#x27; ← 3 = 170&#x27;.
...
single character . (period) is a <ins>name</ins>. Only the first six characters of a
...
name or a pseudoinstruction. A symbol is a valued syllable.
     One may write syllable1&lt;-syllable2 which takes <ins>syllable2</ins> as an integer
which may not be virtual, and must have 0 relocation; the expression means
syllable1 * 2<sup>syllable2</sup>. If either syllable contains any operators whatsoever
it must be a bracketed word. If <ins>syllable1</ins> is virtual, or has ① relocation,
the value of <ins>syllable2</ins> must be less than 2<sup>10</sup>.

If the character &#x27; (single quote) appears to the right of a character in a
symbol, it declares that symbol to be a <ins>variable</ins>. MIDAS forms a list of
...
<ins>global</ins>. If a global symbol is NOT defined in the program where it appears its
</pre>
...
<ins>. AND $.; THE CURRENT LOCATION</ins>

MIDAS maintains an 18-bit quantity (with 1-bit relocation) called the <ins>current
location counter</ins>. Its value, is that address into which the next storage word
...
MIDAS also maintains the <ins>offset</ins>: a 36-bit-&amp;-relocation quantity. This
...
The symbol $.&quot; is virtual. It is the address where the <ins>loader</ins> is about to
...
value of the sixbit,or ASCII code, or ASCII anded with 77<sup>o</sup>, for the
character, respectively. e.g., &#x27;A means 41&#x27;; &quot;+ means 53&#x27;, ↑Q means 21&#x27;

<ins>BRACKETED WORDS</ins>

&lt;word&gt;		is a syllable with the value of <ins>word</ins>, where <ins>word</ins> is a word.
(word)		If the ( is immediately preceded by a syllable-separating
		character, is a syllable whose value is a <ins>word</ins><sub>8</sub>, the value of
		word with left and right halves swapped.
[word]		is a <ins>constant syllable</ins>, and <ins>word</ins> in this case is called a
		<ins>constant word</ins>. As MIDAS assembles a program it forms a list
...
A valued syllable is a <ins>field</ins>; and two or more valued syllables may be combined
...
One or more fields connected by field separators form a <ins>word</ins>. The two field
...
23			   |
24			   A_B			A ⊕ B &amp; 777777
25			   A_B_C		A ⊕ B &amp; 777777 ⊕ C &amp; 777777
26			   A_B,			A + &lt;B &amp; 17&gt; ← 23.
27			   A_B,C		A + &lt;B &amp; 17&gt; ← 23. ⊕ C &amp; 777777
30			   A,			A
31			   not possible
32			   A,,			&lt;A &amp; 777777&gt; ← 18.
...
Anywhere in the word may appear the character @. The only effect of this is
that when the word has been evaluated, the indirect bit, 1←22., is ORed
into the value. The @ does not terminate syllables or fields, nor is it taken
     part of a syllable or field.

Certain symbols form the MIDAS <ins>initial symbol table</ins>. Any of these which represent
...
(Transcriber note: <span title="bent return/down-arrow symbol in original">↵</span> is used instead of the bent downarrow in PDF
     <span title="triangle separator symbol in original">△</span> means triangle symbol used in PDF)

<ins>WORD USES</ins>

name:			makes <ins>name</ins> a symbol with value, relocation and
			virtuality equal to those of the Current Location.

name=word<span title="bent return/down-arrow symbol in original">↵</span>		makes <ins>name</ins> a symbol with value, relocation and
			virtuality of <ins>word</ins>.

name==word<span title="bent return/down-arrow symbol in original">↵</span>		same as <ins>name=word</ins><span title="bent return/down-arrow symbol in original">↵</span>, but also has the effect, when
			the symbol table is read by DDT, of &quot;half-killing&quot;
			the symbol <ins>name</ins> in DDT&#x27;s symbol table.
</pre>
...
word<span title="bent return/down-arrow symbol in original">↵</span>|		     <ins>word</ins> is taken as a storage word.
	  |
word? |		     (a series of <span title="bent return/down-arrow symbol in original">↵</span> characters with no intervening
	 		     <ins>word</ins> creates no storage words and does not change
			     the Current Location Center.

word;text lf	 <ins>word</ins> is taken as a storage word, and the <ins>text</ins> from
	  		     the ; to the first line feed is ignored.


   NOTE: If <span title="bent return/down-arrow symbol in original">↵</span> or ; terminates a word it automatically closes all open ([
   	 and &lt; groupings. Hence ADD_[(3<span title="bent return/down-arrow symbol in original">↵</span> is the same as ADD_[(3)]<span title="bent return/down-arrow symbol in original">↵</span>.


<ins>PSEUDOINSTRUCTIONS  PART I</ins>

     Any arguments supplied to the following pseudoinstructions in excess of
those needed are ignored.

LOC <span title="triangle separator symbol in original">△</span> word <span title="bent return/down-arrow symbol in original">↵</span>	sets the Current Location Counter to the right
    	       		18 bits of the value of <ins>word</ins>, with the relocation
			and virtuality of <ins>word</ins>.

BLOCK <span title="triangle separator symbol in original">△</span> word <span title="bent return/down-arrow symbol in original">↵</span>	sets the Current Location Counter to the right
      	    	 	18 bits of the sum of its previous value and the
			value of <ins>word</ins>

END <span title="triangle separator symbol in original">△</span> word <span title="bent return/down-arrow symbol in original">↵</span>	This marks the end of the program, and the right
...
XWORD <span title="triangle separator symbol in original">△</span> field1 <span title="triangle separator symbol in original">△</span> field2 <span title="bent return/down-arrow symbol in original">↵</span>|-- Same as (,field1),field2
XWD <span title="triangle separator symbol in original">△</span> field1 <span title="triangle separator symbol in original">△</span> field2 <span title="bent return/down-arrow symbol in original">↵</span>  |

EXP <span title="triangle separator symbol in original">△</span> word <span title="bent return/down-arrow symbol in original">↵</span>    Same as word<span title="bent return/down-arrow symbol in original">↵</span>

.OP <span title="triangle separator symbol in original">△</span> field1 <span title="triangle separator symbol in original">△</span> field2 <span title="triangle separator symbol in original">△</span> field3 <span title="triangle separator symbol in original">△</span>
    	  	        This is a <ins>syllable</ins> and so may have syllable-
			combining operators to its left and at the
			right of the last <span title="triangle separator symbol in original">△</span>. The value of the
			syllable is the result of performing <ins>field1</ins>
			as a PDP-6 instruction with <ins>field2</ins> in the
</pre>
...
<pre>
			...
			specified accumulator and <ins>field3</ins> in the
...
RADIX <span title="triangle separator symbol in original">△</span> word<span title="bent return/down-arrow symbol in original">↵</span>	Sets current radix to value of <ins>word</ins>.
...
EQUALS<span title="triangle separator symbol in original">△</span>name1<span title="triangle separator symbol in original">△</span>name2<span title="bent return/down-arrow symbol in original">↵</span>
			<ins>name1</ins> is made a synonym of <ins>name2</ins> and may
			be used anywhere instead of <ins>name2</ins>. This
			pseudoinstruction is generally used upon
			macros and pseudoinstructions rather than
			symbols.

TITLE<span title="triangle separator symbol in original">△</span>string lf	The <ins>string</ins> of characters through the first
		 	line feed is the <ins>title</ins> of the program, and
			is printed out when the TITLE pseudoinstruction
			is encountered on each pass of the assembly.
			The <ins>program name</ins>, which may be used by DDT,
			is set to the first 6 characters of the
			first syllable in the title.

OFFSET<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>	The Offset is set to the value and relocation
			of <ins>word</ins>.

SQOZE<span title="triangle separator symbol in original">△</span>field,symbol	Is a syllable with value equal to the radix-50
			representation of <ins>symbol</ins> plus &lt;field &amp; 17&gt;&lt;-30

ASCII_jtextj		where j is any ASCII character, and <ins>text</ins> is
			...
			<ins>text</ins>, from left to right left-justified in
...
ASCIZ_jtextj		like ASCII, but if number of characters in
			<ins>text</ins> is divisible by 5, assembles final
			additional storage word of 0.

.ASCII jtextj		Like ASCIZ; but also when the character ! is
       			encountered in the <ins>text</ins> takes the <ins>field</ins>
...
SIXBIT_jtextj		Same action as ASCII, but puts six sixbit
...
PRINTC_jtextj		Takes a text argument identical in form to
...
PRINTX_jtextj		Same as PRINTC, but suppresses type-out
			of the character !.

EXPUNG<span title="triangle separator symbol in original">△</span>name1<span title="triangle separator symbol in original">△</span>name2<span title="triangle separator symbol in original">△</span>...<span title="triangle separator symbol in original">△</span>namen<span title="bent return/down-arrow symbol in original">↵</span>
			Each <ins>name</ins> is <ins>expunged</ins>, i.e., deleted from
...
.END			matching .END is termed a <ins>program</ins> block
...
CONI  field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	The eight PDP-6 i-o instructions are defined
CONO  field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	as pseudoinstructions to&#x27;MIDAS. If they
CONSZ field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	appear in the leftmost field of a word, then
CONSO field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	for that word the evaluation rules for
DATAI field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	certain word formats changes according to
DATAO field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	the following table; and the i-o instruction
BLKI  field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	otherwise acts as a symbol with the value
BLKO  field1, field2<span title="bent return/down-arrow symbol in original">↵</span>	shown in the Initial Symbol Table
    	  	       
format number	pattern		value
 <ins>in octal</ins>
26		A_B,		A + &lt;B &amp; 177&gt; &lt;- 26.
27		A_B,C		A + &lt;B &amp; 177&gt; &lt;- 26. ++ C &amp; 777777

.FORMA<span title="triangle separator symbol in original">△</span>no,fs	Inserts an entry in the format table (i.e.,
			replaces old entry) for format number <ins>no</ins>
			...
			handled fields of a word: the left 12 bits
			...
			2 from the right and any additional fields.
			...
			(Example: the 12-bit specification 2704<sub>8</sub>
...
(Transcriber note: digits appended with (8) are octal)

			There are three exceptions to the above
			procedure. (1) If a field is specified
			as 0022<sub>8</sub> (right half, not shifted) the
			carry out of bit 18 is suppressed as the
			field is added into the word. (2) A
			virtual quantity may only occur in a field
			specified 0044<sub>8</sub>,0022<sub>8</sub>,2222<sub>8</sub>, 0504<sub>8</sub> or
			2704<sub>8</sub>. (3) If as a syllable in the left-
...
<ins>PSEUDOINSTRUCTIONS  PART II.</ins>
...
following sections describe the character-string-handling feature of MIDAS,
...
REPEAT<span title="triangle separator symbol in original">△</span>field<span title="triangle separator symbol in original">△</span>text<span title="carriage return">CR</span>	 (where <ins>text</ins> is any character string whose
			  	 first character is not [ and which contains
				 no <ins>cr</ins>) MIDAS detects this pseudoinstruction,
				 determines the numeric value of <ins>field</ins> (as an
				 integer), say n; and passes <ins>text</ins> back as input
				 a total of n times, followed each time by
				 <ins>cr</ins>-<ins>lf</ins>.

REPEAT<span title="triangle separator symbol in original">△</span>field<span title="triangle separator symbol in original">△</span>[text]	 Finds n, the value of <ins>field</ins>; then passes
				 <ins>text</ins> to the input n times. The <ins>text</ins> may
				 ...
				 surrounding <ins>text</ins> are not passed on.
</pre>
...
.RPCNT				This is the symbol whose value depends on
...
The following pseudoinstructions, whose names begin IF, are termed <ins>conditional
assembly</ins> pseudoinstructions.

IF1<span title="triangle separator symbol in original">△</span>text<span title="carriage return">CR</span>		(take <ins>text</ins> argument like that of REPEAT.)
IF1<span title="triangle separator symbol in original">△</span>[text]			The <ins>text</ins> is passed back as input if the
				assembler is performing Pass1. Otherwise
				this usage gives no characters out.

IF2<span title="triangle separator symbol in original">△</span>text<span title="carriage return">CR</span>		Like IF1, but gives <ins>text</ins> back to input only
IF2<span title="triangle separator symbol in original">△</span>[text]			if performing Pass 2

IFE<span title="triangle separator symbol in original">△</span>field<span title="triangle separator symbol in original">△</span>text<span title="carriage return">CR</span>	If the numeric value of <ins>field</ins> is equal to 0,
IFE<span title="triangle separator symbol in original">△</span>field<span title="triangle separator symbol in original">△</span>[text]	the <ins>text</ins> is reprocessed as input; otherwise not.
...
IFSE<span title="triangle separator symbol in original">△</span>string1<span title="triangle separator symbol in original">△</span>string2<span title="triangle separator symbol in original">△</span>text<span title="carriage return">CR</span>
IFSE<span title="triangle separator symbol in original">△</span>string1<span title="triangle separator symbol in original">△</span>string2<span title="triangle separator symbol in original">△</span>[text]
				...
				the string comparison. The <ins>text</ins> is then
				processed only if the two fields are
				identical character strings.

IFSN				Like IFSE, but passes text on only if the
				two fields are not identical strings.

IRP<span title="triangle separator symbol in original">△</span>A, B, [P,Q,R]<span title="bent return/down-arrow symbol in original">↵</span>	The IRP pseudoinstruction takes a <ins>triplet</ins> of
	      			arguments, separated by commas: the first
				two are names, called <ins>dummy symbols</ins>; the third
				is termed a <ins>list</ins>. Each of the dummy symbol
				names need not be given. The list consists
				of an open bracket, any number of <ins>elements</ins>
</pre>
...
TERMIN				The <ins>range</ins> of an IRP is the string of
				...
				the list to the IRP. The first time
				through the range, the first dummy symbol
				of the IRP has the <ins>string value</ins> of the
...
will recirculate the string
	 ...
	 ADD 3, W4
				...
				each iteration through the range, the string
				value of the remainder of the list
				yet unused: for instance, on the third
				iteration of IRP Q,R,[AND, IOR, XOR, EQV,
				ANDCA] for R would be substituted EQV,ANDCA.

IRP A,B,[X,Y,Z]C,D,[P,Q,R]E,F,[S,T,U] ...<span title="bent return/down-arrow symbol in original">↵</span>
...
<pre>
				...
				symbols receive null string values.

IRPC<span title="triangle separator symbol in original">△</span>A,B,[string]<span title="bent return/down-arrow symbol in original">↵</span>	Here A and B are dummy symbols, and <ins>string</ins>
				is a string of characters ended by the
				first matching ]. On each iteration of
				the IRPC the first dummy symbol takes the
				string value of each successive <ins>character</ins>
				in <ins>string</ins>, and the second dummy symbol has
				the string value of the string to the right
				of that character. In all other respects
				IRPC is used identically to IRP.

IRPS<span title="triangle separator symbol in original">△</span>A,B,[syl1<span title="triangle separator symbol in original">△</span>syl2<span title="triangle separator symbol in original">△</span>...<span title="triangle separator symbol in original">△</span>syln]
				Again A and B are dummy symbols; <span title="triangle separator symbol in original">△</span> is
				any field or syllable separator. On each
				iteration the first dummy symbol has the strin
				value of successively each of the <ins>syl<sub>i</sub></ins>, and th
				second dummy has the string value if th
				<span title="triangle separator symbol in original">△</span> to the right of the syllable which is the
				current value of the first dummy symbol. Othe
				wise IRPS is like IRP and IRPC.

<ins>MACROINSTRUCTIONS</ins>

A <ins>macroinstruction definition<ins> is a syntactic element consisting of: a define line; 
a definition body; and the pseudoinstruction TERMIN.

A <ins>define line</ins> contains in order: the pseudoinstruction DEFINE; a name, which
is to be defined, called the <ins>macro name</ins>; and zero or more names which are dummy
...
<span title="bent return/down-arrow symbol in original">↵</span> character, but if that character is <span title="carriage return"><ins>cr</ins></span> and the next character is <span title="line feed"><ins>lf</ins></span> the <span title="line feed"><ins>lf</ins></span>
...
The <ins>definition body</ins> is all text following the define line, up to but not
including the first unmatched use of TERMIN. (The TERMIN pseudoinstruction is
matched by IRP, IRPC, IRPS and DEFINE.)

The effect of a macroinstruction definition is that when its TERMIN is encountered
the macro name becomes defined as a macroinstruction with the <ins>string value</ins> of the
definition body.

As this happens, the characters to the left and right of each dummy
symbol occurence in the body are examined: each of which is an <ins>exclamation
point</ins> is deleted and its place taken by a mark which is invisible; except
...
are performed:
    ...
    (I)  the characters following the separator which ends
    	 the macro name are read as a list of <ins>arguments</ins>,
	 separated by comma and the list ended by carriage
	 return or semicolon. This process, termed the
	 <ins>argument scan</ins>, associates in order each dummy symbol in
...
<pre>
	 ...
	 <span title="bent return/down-arrow symbol in original">↵</span> character; if there are dummy symbols to the
	 ...
	 expected is [ (open bracket), that argument will
...
<pre>
	 ...
	 rules:
		...
		1) If there is no / or \ in the list of dummy
		...
		2) If there is one or more / or \ in the list
		   ...
		   G00001 or G00002, etc. (called a <ins>generated
		   symbol</ins>; a unique generated symbol being
...
<pre>
    ...
    (II) The effect to the assembler is that the macro name
	 ...
	 For example, if MAC is defined as follows
	      ...
	      DEFINE_MAC_E,F<span title="bent return/down-arrow symbol in original">↵</span>
	      →|MOVEI_E,3<span title="bent return/down-arrow symbol in original">↵</span>
	      →|IMULB_E,F<span title="bent return/down-arrow symbol in original">↵</span>
	      TERMIN
	 ...
	 The character ! may be used to delimit dummy symbols when the effect of any
	 other syllable separator is not wanted. The definition

	      DEFINE HAC M,E,F<span title="bent return/down-arrow symbol in original">↵</span>
	      →|MOVEI_E,3<span title="bent return/down-arrow symbol in original">↵</span>
	      →|IMUL!M_E,F<span title="bent return/down-arrow symbol in original">↵</span>
	      TERMIN

	 enables one to then say HAC_B,1,LOC to generate
	      -&gt;|MOVEI_1,3
	      -&gt;|IMULB_1,LOC

	 or HAC_,1,LOC to give
	      -&gt;|MOVEI_1,3
	      -&gt;|IMUL_1,LOC
</pre>
...
<pre>
	...
	the macro name is lost: HAC__,1,LOC has a first argument consisting of
	one space.

	    A common use of \ preceding an argument is as follows:

	      	     DEFINE_DISPATCH N,TAB
		     	JRST_TAB!N
		     TERMIN
		     ..=1
	...
	within the definition of the macro being called:
			...
			TERMIN
		     ...
		     <span title="down-arrow symbol in original">↓</span>
		     MAPONE MAC,[1,LOC]
		     MAPONE MAC,[1,LOK]
		     MAPONE MAC,[2,LOG]
		     <span title="down-arrow symbol in original">↓</span>
...
.TAG<span title="triangle separator symbol in original">△</span>name<span title="right-facing triangle symbol in original">▷</span> may be used within the range of IRP, IRPS or IRPC; or in the
.GO<span title="triangle separator symbol in original">△</span>name<span title="right-facing triangle symbol in original">▷</span>  body of a macro definition. Only the first 6 characters of
...
.GSSET<span title="triangle separator symbol in original">△</span>no<span title="bent return/down-arrow symbol in original">↵</span>	Sets generated-symbol counter to value of no

<ins>PSEUDOINSTRUCTIONS PART III</ins>
...
MACDMP, or HACTRN (the DDT in time sharing) and on paper tape is preceded by
...
RIM<span title="bent return/down-arrow symbol in original">↵</span>     	      selects read-in mode format output
RIM1<span title="bent return/down-arrow symbol in original">↵</span>    	      selects half-word read-in mode format output
SBLK<span title="bent return/down-arrow symbol in original">↵</span>    	      selects simple block format output
RELOCA<span title="bent return/down-arrow symbol in original">↵</span>  	      selects relocatable format output and sets
	     	      current location counter to 0 with relocation 1.
1PASS<span title="bent return/down-arrow symbol in original">↵</span>   	      like RELOCA, but also causes 1-pass assembly.
SBLK is initially selected.
.SLDR		      causes output of SBLK loader and enters SBLK format
WORD<span title="triangle separator symbol in original">△</span>wrd<span title="bent return/down-arrow symbol in original">↵</span>     The value of <ins>wrd</ins> is output as a 36-bit word. In
		      ...
		      <ins>wrd</ins> appears in the proper place and between blocks.
</pre>
...
.LIBRA_namelist<span title="bent return/down-arrow symbol in original">↵</span>  This must occur before any storage words. It
		      tells the loader that the program to follow is
		      a <ins>library program</ins>. The entries in the <ins>namelist</ins>,
		      separated by commas, are either names or groups of
		      names separated by space, +, and -. An entry is
		      said to be <ins>satisfied</ins> by a list of symbols if
		      either (a) the entry is a name and that name appears
		      in the list of symbols; or (b) the entry is a group
		      of names, <ins>all</ins> of which names preceded by space or +
		      are in the list of symbols, and <ins>none</ins> of which names
		      preceded by - are. The Linking Loader will omit to
		      load a library program unless one or more entries in
		      its namelist are satisfied by the list of undefined
		      global symbols used in programs already loaded.

.LIFS_namelist<span title="bent return/down-arrow symbol in original">↵</span>   Load If Seen.

.ELDC<span title="bent return/down-arrow symbol in original">↵</span>	      End Load Time Conditional. Any storage words
		      ...
		      list of defined <ins>and</ins> undefined global symbols used
		      ...
		      only be used after the last. Notice the LOC $.&quot;
...
.LIFE<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>   Load Time Conditionals on Value. Any one of
.LIFLE<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>  these starts a range which is ended by .ELDC
.LIFL<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>   In each case the <ins>word</ins> is evaluated by the Loader,
.LIFG<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>   and the storage words in the range are loaded
.LIFGE<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>  only if the value of <ins>word</ins> is respectively
.LIFN<span title="triangle separator symbol in original">△</span>word<span title="bent return/down-arrow symbol in original">↵</span>   = 0, &lt;= 0, &lt; 0, &gt; 0, &gt;= 0, neq 0.

.LNKOT		      If output is relocatable format, causes
		      immediate output of all accumulated linking
		      pointers.

.LOP<span title="triangle separator symbol in original">△</span>f1<span title="triangle separator symbol in original">△</span>f2<span title="triangle separator symbol in original">△</span>f3
...
.GLOBA<span title="triangle separator symbol in original">△</span>nam1<span title="triangle separator symbol in original">△</span>nam2,...<span title="bent return/down-arrow symbol in original">↵</span>
		      has same effect as appearance of each name
		      followed by a &quot;, but generates no storage
		      words.

.LIBRQ<span title="triangle separator symbol in original">△</span>nam1<span title="triangle separator symbol in original">△</span>,...<span title="bent return/down-arrow symbol in original">↵</span>
...
(if any); CCC is a 3-character error code, as listed below. A dagger + in the
following listing marks a <ins>fatal</ins> error, which MIDAS will not proceed past.
...
MDT		multiply defined tag. A variable, defined symbol or
		macroinstruction appeared to the
		left of : and was not redefined.

MDV		multiply defined variable. A defined symbol
...
IRC		illegal relocation count. A field had a relocation
		other than +1 or 0.
...
+CLD		constant word area differs from
		previous pass

+VLD		variable area location differs from previous pass

+CRD		relocation of constant word area differs from
		previous pass
...
<ins>OPERATING INSTRUCTIONS NON TIME-SHARING</ins>
...
appreciated as an <ins>in vivo</ins> check-out of the new version.) When MIDAS is started,
...
(<span title="ALT MODE / escape">$⃝</span> will be used as a symbol for ALT MODE.) In normal use, one should give
...
(2) may be interchanged).
    ...
    (1) Input Selection: the command nERname1_name2<span title="ALT MODE / escape">$⃝</span>
    will select for input the file named <ins>name1</ins> <ins>name2</ins>
    or the DECtape on unit <ins>n</ins>. If the ER command is
    ...
    output on the DECtape on unit <ins>m</ins>. If the EI command
...
<pre>
    ...
    I		Initialize. Forget all symbols, macros, etc
    		except for those in the Initial Symbol Table:
...
<pre>
     ...
     (4)  Output Filing: the command EF name1_name2<span title="ALT MODE / escape">ESC</span>
     	  will file DECtape output in SBLK mode for
	  absolute-mode output or RELOC mode for
	  relocatable mode output. There is no need
	  for this output if output was to paper tape, or if
...
There are also several characters which may be typed in and take effect
immediately, without being part of command strings. These are various letters
with the CTRL key hold:

(Transcriber note: Original PDF uses a notation that encloses the control
character in an "angle". Since there are no unicode characters the ^CHAR notation is used.)
...
^G   (bell)   Quit current activities and wait for typed-in
...
<ins>OPERATING INSTRUCTIONS TIME SHARING</ins>

To DDT type MIDAS ^H (alternatively, any other commands which will load
...
<span title="left-arrow symbol in original">←</span> (shift 0); (c) the source file designation; (d) a carriage return. A
...
subname is the first subname of the source file, and whose second subname
...
AOBJN		(252000)		BLKO		(700100) pseudo
					        BLOCK		pseudo
<ins>AOBJP		(252000)		BLT		    (251000)</ins>
...
FSBB		(153000)	    |	HLLZS		(514000)
<ins>FSBL		(151000)	    |	HLR		    (544000)</ins>
...
<ins>HRLO		(524000)	|	HRRZM		(552000)</ins>
...
<ins>IOR		    (434000)	|	MOVEI		(201000)</ins>
...
<ins>ORCM		(464000)	|	SETCAI		(451000)</ins>
...
<pre>
					...
					42.
						                    ...
						                    for example, system
						                    calls, status bits
Symbol	Value       | Symbol	Value		etc. This list
		            |    			        changes frequently
...
TDOE	(672000)    | TLZN	    (627000)	conflict with these
...
<ins>TLN	    (601000)    | TRON    	(666000)</ins>
TRZ	    (620000)    | UTS	    214
...
TSON	(677000)    | .FNAM2	pseudo (t-s only)
...
<ins>SBLK</ins>
	...
	(1) SBLK loader in RIM format: Read <ins>Left-to-Right</ins>
	    	 	* numbers are octal *
...
for each data	    |	   		| contents of 1st address.   |
block is 	        |	   		| contents of (1st addr.+1)	 |    n data
computes as	        |	   		| .	      	      		     |      words
follows: take	    |	   		| .				             |
the header	        |	   		| contents of (1st addr.+n-1)|
word: rotate	    |	logical | checksum
it to the	        |   block		  .
left one bit PROGRAM|	------------- .
and add the	        |	MIDAS		  any number of data blocks
first data	        |	punched		  -
word; rotate	    |	blocks		  -
the result to	    |	have		  -
the left one	    |	n&lt;= 40&#x27;		  JRST start			; just block
bit and add	        |	    		       				              to start
the second	        |							                      of program
data word; etc.	    |
After the nth	    |
data word the	    |
result should	    |
...
in the header	    |	      |	SQUOZE 0(040000)  |
word of each	    |	      |	value  		      | 1st symbol
data block	        |	      |	SQUOZE+(040000)   +
of the symbol	    |	      |	value		      | 2nd symbol
table should	    |	      | .	 
be ignored	        |	      |	.	
	SYMBOL	        |	block |	SQUOZE+(040000)	  | (m/2)th symbol
	TABLE	        |	      | value		      |
Notice that	        |	      | checksum
a JRST 		        |		      .
instruction	        |		     any number of blocks
is positive	        |		    JRST 0		           ; end of symbols
</pre>
...
<pre>
						47.
					<ins>Suggested Symbolic Format</ins>
...
to good practice.
   ...
   (3)  All address tags referring to a given location should appear on
        separate lines, with the contents of that location once
        on the same line as the last (or only) tag.
   (4)  Blank lines (redundant carriage returns, line feeds, etc) may
        occur whenever useful to demarcate parts of a routine.
...
first page of a program which uses a TV camera.) It demonstrates many,
