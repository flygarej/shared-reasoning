...
nMname1,name2($)   Set to read from the beginning of file name1, name2 on
...
leftarrow    	|
uparrow		|--------------	intra-syllable operators
...
form and upon how it was defined, a syllable may be a number, a symbol,
a macro name, a pseudoinstruction, a quoted character or one of several kinds
...
valued syllables) represent numeric quantities, which are 36 bits in size.
...
some syllables may be virtual, meaning the value is not known at assembly
...
by . (period) is taken in base 10. An integer may be followed by (uparrow) followed
by an integer:
   A (uparrow) B where A and B are integers, means the integer
   A*R(subscript A) (superscript B) with R(subscript A) being the radix in which A is expressed.
...
(uparrow) integer, and the result is a floating point number: eg 3.5 (uparrow) 4 means the
</pre>
...
which multiplies the current value by 2(raised to) INTEGER. (The exponent integer may
...
name or a pseudoinstruction. A symbol is a valued syllable.
     One may write syllable1&lt;-syllable2 which takes syllable2 as an integer
which may not be virtual, and must have 0 relocation; the expression means
syllable1 * 2 raised to power of syllable2. If either syllable contains any operators whatsoever
it must be a bracketed word. If syllable1 is virtual, or has (1 in circle) relocation,
the value of syllable2 must be less than 2(to tenth power).
...
If &#x27;(single quote) or &quot;(double quote) or (uparrow)(up arrow) appears without
...
value of the sixbit,or ASCII code, or ASCII anded with 77(octal), for the
character, respectively. e.g., &#x27;A means 41&#x27;; &quot;+ means 53&#x27;, (uparrow)Q means 21&#x27;
...
(word)		If the ( is immediately preceded by a syllable-separating
		character, is a syllable whose value is a word(8), the value of
...
the following chart.
		...
		(Transcriber: _ stands for space, ++ stands for
		 addition with carry suppressed from bit 18 to bit 17
		 &lt;- is leftarrow)
...
The sign (+ in circle) means addition with carry suppressed from bit 18 to bit 17
</pre>
...
.FORMA<span title="triangle separator symbol in original">△</span>no,fs	Inserts an entry in the format table (i.e.,
			...
			(Example: the 12-bit specification 2704 (octal)
...
(Transcriber note: digits appended with (8) are octal)
			...
			2704(8) is instead taken as if specified
...
(2) may be interchanged).
    ...
    (1) Input Selection: the command nERname1_name2<span title="ALT MODE / escape">ESC</span>
