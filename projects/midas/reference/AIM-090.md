...
> **Layout-preserving Markdown working edition**
>
> Derived from the diplomatic transcription. The transcription remains authoritative
> relative to this Markdown edition, and the scanned document remains the ultimate
> authority. No spelling, punctuation, capitalization, apparent typographical errors,
> placeholder tags, indentation, or line breaks in the transcription have been
> silently corrected in this pass.
>
> The transcription is enclosed as preformatted text so that its existing layout is
> preserved. Structural Markdown and typographical substitutions can be introduced
> incrementally in later passes and checked against the diplomatic transcription.
...
This is a transcription of MIT Project MAC Artificial Intelligence
Memo No. 90, "MIDAS", by Peter Samson.

The scanned document is authoritative. Original spelling, punctuation,
capitalization, and apparent typographical errors are preserved.

Text supplied or reconstructed by the transcriber is explicitly marked.
A question mark <?> following reconstructed text indicates uncertainty.

Page breaks in the source are preserved.

Several handdrawn typgraphical symbols are in the PDF, those have been given
a tag and are referred in text as <tag>, exception is when space is explicitly
marked in text, then _ (underscore) is used.
...
(a1)	To load the program APHLA RALPHA from DECtape unit 3, the program
	SUBR 1 from unit 2 and the program SUBR 2 from unit 2; then to (Transcriber
	note: original text is 'BR for SUBR 2)
...
(Transcriber note: <ret> is used instead of the bent downarrow in PDF
     <tri> means triangle symbol used in PDF)
...
name=word<ret>		makes name a symbol with value, relocation and
			virtuality of word.

name==word<ret>		same as name=word<ret>, but also has the effect, when
...
word<ret>|		word is taken as a storage word.
	 |
word?	 |		(a series of <ret> characters with no intervening
...
word;text lf		word is taken as a storage word, and the text from
   ...
   NOTE: If <ret> or ; terminates a word it automatically closes all open ([
   	 and < groupings. Hence ADD_[(3<ret> is the same as ADD_[(3)]<ret>.
...
LOC <tri> word <ret>	sets the Current Location Counter to the right
    	       		18 bits of the value of word, with the relocation
			and virtuality of word.

BLOCK <tri> word <ret>	sets the Current Location Counter to the right
      	    	 	18 bits of the sum of its previous value and the
			value of word

END <tri> word <ret>	This marks the end of the program, and the right
...
XWORD <tri> field1 <tri> field2 <ret>|-- Same as (,field1),field2
XWD <tri> field1 <tri> field2 <ret>  |

EXP <tri> word <ret>    Same as word<ret>

.OP <tri> field1 <tri> field2 <tri> field3 <tri>
    	  	        This is a syllable and so may have syllable-
			combining operators to its left and at the
			right of the last <tri>. The value of the
...
RADIX <tri> word<ret>	Sets current radix to value of word.
...
EQUALS<tri>name1<tri>name2<ret>
...
TITLE<tri>string lf	The string of characters through the first
...
OFFSET<tri>word<ret>	The Offset is set to the value and relocation
			of word.

SQOZE<tri>field,symbol	Is a syllable with value equal to the radix-50
...
EXPUNG<tri>name1<tri>name2<tri>...<tri>namen<ret>
...
CONI  field1, field2<ret>	The eight PDP-6 i-o instructions are defined
CONO  field1, field2<ret>	as pseudoinstructions to'MIDAS. If they
CONSZ field1, field2<ret>	appear in the leftmost field of a word, then
CONSO field1, field2<ret>	for that word the evaluation rules for
DATAI field1, field2<ret>	certain word formats changes according to
DATAO field1, field2<ret>	the following table; and the i-o instruction
BLKI  field1, field2<ret>	otherwise acts as a symbol with the value
BLKO  field1, field2<ret>	shown in the Initial Symbol Table
...
.FORMA<tri>no,fs	Inserts an entry in the format table (i.e.,
...
REPEAT<tri>field<tri>text<cr>	 (where text is any character string whose
...
REPEAT<tri>field<tri>[text]	 Finds n, the value of field; then passes
...
IF1<tri>text<cr>		(take text argument like that of REPEAT.)
IF1<tri>[text]			The text is passed back as input if the
				assembler is performing Pass1. Otherwise
				this usage gives no characters out.

IF2<tri>text<cr>		Like IF1, but gives text back to input only
IF2<tri>[text]			if performing Pass 2

IFE<tri>field<tri>text<cr>	If the numeric value of field is equal to 0,
IFE<tri>field<tri>[text]	the text is reprocessed as input; otherwise not.
...
IFSE<tri>string1<tri>string2<tri>text<cr>
IFSE<tri>string1<tri>string2<tri>[text]
...
IRP<tri>A, B, [P,Q,R]<ret>	The IRP pseudoinstruction takes a triplet of
...
IRP A,B,[X,Y,Z]C,D,[P,Q,R]E,F,[S,T,U] ...<ret>
...
IRPC<tri>A,B,[string]<ret>	Here A and B are dummy symbols, and string
...
IRPS<tri>A,B,[syl1<tri>syl2<tri>...<tri>syln]
				Again A and B are dummy symbols; <tri> is
				...
				<tri> to the right of the syllable which is the
...
<ret> character, but if that character is <cr> and the next character is <lf> the <lf>
...

	 ...
	 <ret> character; if there are dummy symbols to the
...
.TAG<tri>name<rtri> may be used within the range of IRP, IRPS or IRPC; or in the
.GO<tri>name<rtri>  body of a macro definition. Only the first 6 characters of
...
.GSSET<tri>no<ret>	Sets generated-symbol counter to value of no
...
RIM<ret>     	      selects read-in mode format output
RIM1<ret>    	      selects half-word read-in mode format output
SBLK<ret>    	      selects simple block format output
RELOCA<ret>  	      selects relocatable format output and sets
	     	      current location counter to 0 with relocation 1.
1PASS<ret>   	      like RELOCA, but also causes 1-pass assembly.
SBLK is initially selected.
.SLDR		      causes output of SBLK loader and enters SBLK format
WORD<tri>wrd<ret>     The value of wrd is output as a 36-bit word. In
...
.LIBRA_namelist<ret>  This must occur before any storage words. It
...
.LIFS_namelist<ret>   Load If Seen.

.ELDC<ret>	      End Load Time Conditional. Any storage words
...
.LIFE<tri>word<ret>   Load Time Conditionals on Value. Any one of
.LIFLE<tri>word<ret>  these starts a range which is ended by .ELDC
.LIFL<tri>word<ret>   In each case the word is evaluated by the Loader,
.LIFG<tri>word<ret>   and the storage words in the range are loaded
.LIFGE<tri>word<ret>  only if the value of word is respectively
.LIFN<tri>word<ret>   = 0, <= 0, < 0, > 0, >= 0, neq 0.

.LNKOT		      If output is relocatable format, causes
		      immediate output of all accumulated linking
		      pointers.

.LOP<tri>f1<tri>f2<tri>f3
...
(<esc> will be used as a symbol for ALT MODE.) In normal use, one should give
...
(2) may be interchanged).
    ...
    (1) Input Selection: the command nERname1_name2<esc>
...

     ...
     (4)  Output Filing: the command EF name1_name2<esc>
...
<l-arrow> (shift 0); (c) the source file designation; (d) a carriage return. A
...
though by no means all, of the features of MIDAS. The FIX macro is intended a
